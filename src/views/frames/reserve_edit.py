...
class FrameReserveEdit(ctk.CTkScrollableFrame):
    def __init__(self, parent, reserve_id):
        ...
        ctk.CTkLabel(self.form_frame, text=self.reservation[1]).grid(row=2, column=0, pady=(3, 0), padx=20, sticky="w")

        # DateTimeStart
        ctk.CTkLabel(self.form_frame, text="Hora de Inicio").grid(row=3, column=0, padx=20, pady=(20, 0), sticky="w")
        ctk.CTkLabel(self.form_frame, text=self.reservation[2]).grid(row=4, column=0, padx=20, pady=(20, 0), sticky="w")

        # DateTimeEnd
        ctk.CTkLabel(self.form_frame, text="Hora de Fim").grid(row=3, column=3, padx=20, pady=(20, 0), sticky="w")
        ctk.CTkLabel(self.form_frame, text=self.reservation[3]).grid(row=4, column=3, padx=20, pady=(20, 0), sticky="w")
        ...
        self.combo = ctk.CTkComboBox(self, values=ReservationStatus.get_status_edit(), variable=self.category, width=200)
        self.combo.grid(row=11, column=3, pady=(3, 0), padx=20, sticky="w")
        self.category_error = ctk.CTkLabel(self, text="", text_color="red")
        self.category_error.grid(row=12, column=3, pady=0, padx=20, sticky="w")
