...
class FrameUserNew(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.user_type = ctk.StringVar(self, UserPriority.get_user_priorities()[0])

        # Email field
        ctk.CTkLabel(self, text="Email").grid(row=1, column=0, padx=20, pady=(20, 0), sticky="w")
        self.email = ctk.CTkEntry(self, width=200)
        self.email.grid(row=2, column=0, pady=(3, 0), padx=20, sticky="w")
        self.email_error = ctk.CTkLabel(self, text="", text_color="red")
        self.email_error.grid(row=3, column=0, pady=0, padx=20, sticky="w")

        # Type field
        ctk.CTkLabel(self, text="Tipo de Utilizador").grid(row=1, column=3, padx=20, pady=(20, 0), sticky="w")
        combo = ctk.CTkComboBox(self, values=UserPriority.get_user_priorities(), variable=self.user_type, width=200)
        combo.grid(row=2, column=3, pady=(3, 0), padx=20, sticky="w")

        # Cellphone field
        ctk.CTkLabel(self, text="Telemóvel").grid(row=5, column=0, padx=20, pady=(20, 0), sticky="w")
        self.cellphone = ctk.CTkEntry(self, width=200)
        self.cellphone.grid(row=6, column=0, pady=(3, 0), padx=20, sticky="w")
        self.cellphone_error = ctk.CTkLabel(self, text="", text_color="red")
        self.cellphone_error.grid(row=7, column=0, pady=0, padx=20, sticky="w")

        # Submit button
        self.button = ctk.CTkButton(self, text="Submeter", command=self.submit, width=200)
        self.button.grid(row=9, column=3, padx=20, pady=(30, 5), sticky="e")
    ...
    def submit(self) -> None:
        ...
        """
        if self.is_valid():
            UserDI.add_user(self.user_type.get(), self.email.get(), self.cellphone.get())
